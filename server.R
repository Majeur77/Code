...
scoreResult = function(syllabus_file){
  
  case <- read.csv(file.path("courseDescription.csv"),stringsAsFactors = F)
...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$upload_syllabus,{
    ...
    case_resume_result <- scoreResult(input$upload_syllabus$datapath)
    ...
    output$downloadData <- downloadHandler(
      ...
      content = function(file1){
        write_xlsx(case_resume_result,file1)
